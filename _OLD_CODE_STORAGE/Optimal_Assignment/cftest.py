...
class hungarian:
    ...
    def step_four(self):
        ...
        print('----last maximum matching list: {}'.format(self.match_list))
        print('----last self.L: {}'.format(self.L))
        row_remain = [i for i in range(self.N)]
        column_remain = [i for i in range(self.N)]
        row_removed = []
        column_removed = []
        #print('----row_remain: {}'.format(row_remain))
        #print('----col_remain: {}'.format(column_remain))
        for i in self.match_list:
            row_remain.remove(i[0])
            row_removed.append(i[0])
        print('----first, only remove rows:row_remain: {}'.format(row_remain))
        print('----first, only remove rows:row_removed: {}'.format(row_removed))
        for i in row_remain:
            for j in self.L[i]:
        ...
        print('----after edit rows:row_remain: {}'.format(row_remain))
        print('----after edit remove rows:row_removed: {}'.format(row_removed))


        print('----col_remain: {}'.format(column_remain))
        print('----column_removed: {}'.format(column_removed))
        tem = [self.minimum_value_matching_matrix[i][j] for i in row_remain for j in column_remain]
        print('----remain list: {}'.format(tem))
        mini_value_4 = min(tem)
        print('----minimum value in remain list is {}'.format(mini_value_4))
        for i in row_remain:
            for j in column_remain:
                self.minimum_value_matching_matrix[i][j] = self.minimum_value_matching_matrix[i][j] - mini_value_4
        print('----Only updated remain value mini matrix:\n {}'.format(np.array(self.minimum_value_matching_matrix)))
        for i in row_removed:
            for j in column_removed:
                self.minimum_value_matching_matrix[i][j] = self.minimum_value_matching_matrix[i][j] + mini_value_4
        print('----new mini matrix:\n {}'.format(np.array(self.minimum_value_matching_matrix)))
        print('----sending this mini matrix back to step 3')
        print('----END step 444444444444444444444444444444444')
        self.step_three()

    def step_five(self):
        print('-----5 555555555555555555555555555555')
        optimal_value = 0
        a=[]
        for i in self.match_list:
            optimal_value += self.matrix[i[0]][i[1]]
            a.append(' '.join((str(i[0]),str(i[1]))))
            #b = '\n'.join(a)
        print(a)
        print('for'*10)
        for i in a:
