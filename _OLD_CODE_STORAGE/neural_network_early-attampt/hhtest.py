#why? lower nodes and layers do better? like 2x2
#it can't compensate 6x6 size....why?????????????????????????????
import numpy as np
class NN():
    def __init__(self, node = 4, layer = 4):
        self.isNew = True
    ...
    def backward(self):
        for i in range(self.N):
            n = self.N-1-i
            if n == self.N-1:
                self.delta_W[n] = (-1)*( np.subtract(self.Y,self.L[self.N-1]) )
            elif n == 0:
