...
for trial in range(MAX_TRIALS):
    try:
        ...
        recording_obs = np.array(recording_obs, dtype=np.float32)   # shape: (episode length, 3, 42, 42)
        recording_action = np.array(recording_action, dtype=np.uint8)   # shape: (episode length, )
