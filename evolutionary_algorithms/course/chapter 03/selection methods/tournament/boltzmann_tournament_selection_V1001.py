...
def evaluation():
    ...
    # fitness = sigma_truncation(fitness)
...
def selection(gen):
    ...
    k_s = 2
    alpha_in = 4
    t_in = alpha_in * (1.0000001 - (gen / maxgen))

    for i in range(mu):
        t1 = np.random.randint(0, mu)
        t2 = np.random.randint(0, mu)
        # while t2 == t1:
        #     t2 = np.random.randint(0, mu)
        prob_in = 1 / (1+(np.exp((fitness[t2]-fitness[t1])/t_in)))
        if np.random.random() < prob_in:
            pool.append(t1)
        else:
            pool.append(t2)
