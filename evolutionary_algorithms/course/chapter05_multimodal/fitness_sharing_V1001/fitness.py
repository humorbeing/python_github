...
fitness_sharing_sigma = 2
fitness_sharing_alpha = 2
# ------------------------
def d(cand_in, i, j):
    vector_in = cand_in[i]-cand_in[j]
    return np.linalg.norm(vector_in)
def s(cand_in, i, j, sigma=fitness_sharing_sigma,
      alpha=fitness_sharing_alpha):
    dis = d(cand_in, i, j)
    if dis < sigma:
        return 1-((dis/sigma)**alpha)
    else:
        return 0
def m(cand_in, i):
    mi = 0
    for j in range(len(cand_in)):
        mi += s(cand_in, i, j)
    return mi
...
def get_fitness(cand_in, obj, reverse=False, fitness_sharing=True):
    fit = []
    for i in range(len(cand_in)):
        f = obj(cand_in[i])
        if fitness_sharing:
            mi = m(cand_in, i)
            f = f/mi
