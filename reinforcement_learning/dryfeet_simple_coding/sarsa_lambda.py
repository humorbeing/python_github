...
def get_an_action(s, epsilon=0.9):
...
def sarsa_lambda(k):
    ...
    for iter in range(k):
        ...
        for up_s in states:
            for up_a in move:
                Q[up_s[0]][up_s[1]][up_a] = Q[up_s[0]][up_s[1]][up_a] + alpha * delta * E[up_s[0]][up_s[1]][up_a]
                E[up_s[0]][up_s[1]][up_a] = gamma * lambda_here * E[up_s[0]][up_s[1]][up_a]
...
def argmaxQ():
    for s in states:
        ...
        num_max = 0
        for a in Q[s[0]][s[1]]:
            if max_v == Q[s[0]][s[1]][a]:
                num_max += 1
            POLICY[s[0]][s[1]][a] = 0.0
        probability = 1 / num_max
        for a in Q[s[0]][s[1]]:
            if max_v == Q[s[0]][s[1]][a]:
                POLICY[s[0]][s[1]][a] = probability
