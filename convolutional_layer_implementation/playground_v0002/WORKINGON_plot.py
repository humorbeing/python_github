...
def logg(name):
    ...
    for line in lines:
        ...
        lo = l_line.split(' ')
        lo = float(lo[2])
        log_this.append([ep, lo])

    log_this = np.array(log_this).T
    return log_this
# print(log_this)
a = logg('r1.txt')
b = logg('r2.txt')
c = logg('r3.txt')
fig, ax = plt.subplots()
ax.plot(a[0], a[1],label='1 per class')
ax.plot(b[0], b[1],label='10 per class')
ax.plot(c[0], c[1],label='only one image')
ax.grid(True)
...
fig, ax = plt.subplots()
plot_this('en_noload_nofreeze_normal_train.txt', 'A3C', 'm', 4)
...
ax.set_xlim(left=0, right=5000000*4.2)
